...
HRESULT PrimeSenseSensor::createFirstConnected()
{
	...
	int deviceNumber = deviceInfoList.getSize();


	// Create Device
	//rc = m_device.open(deviceURI);
	rc = m_device.open(deviceInfoList[0].getUri());
	...
	//std::cout << "Color video modes: " << std::endl;
	...
	m_depthStream.setVideoMode(depthSensorInfo->getSupportedVideoModes()[0]);
	m_colorStream.setVideoMode(colorSensorInfo->getSupportedVideoModes()[0]);
	if (rc == openni::STATUS_OK)
	{
	...
	std::cout << "Depth: (" << depthWidth << "," << depthHeight << ")" << std::endl;
	std::cout << "Color: (" << colorWidth << "," << colorHeight << ")" << std::endl;
