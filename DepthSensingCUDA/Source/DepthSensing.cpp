...
int main(int argc, char** argv)
{
	...
	try {
		...
		std::string fileNameDescGlobalCameraPoseOpt;
		if (argc == 3 || argc == 4) {
			fileNameDescGlobalApp = std::string(argv[1]);
			fileNameDescGlobalTracking = std::string(argv[2]);
			if (argc == 4)
			{
				fileNameDescGlobalCameraPoseOpt = std::string(argv[3]);
			}
			
		} else {
			std::cout << "usage: DepthSensing [fileNameDescGlobalApp] [fileNameDescGlobalTracking] ([fileNameDescCameraPoseOpt])" << std::endl;
			...
			// CHAO: new parameter file for camera pose optimization process
			fileNameDescGlobalCameraPoseOpt = "zParametersCameraPoseOpt.txt";
		}
		...
		std::cout << VAR_NAME(fileNameDescGlobalCameraPoseOpt) << " = " << fileNameDescGlobalCameraPoseOpt << std::endl;
		std::cout << std::endl;
...
void reconstruction()
{
	//only if binary dump (for BinaryDumpReader)
	if (GlobalAppState::get().s_sensorIdx == 3) 
	{
			...
			if (!GlobalAppState::get().s_trackingEnabled) {
				...
				if (!useRGBDTracking) {
					...
					transformation = g_cameraTrackingRGBD->applyCT(
						...
						g_rayCast->getRayCastData().d_colors, //g_CudaDepthSensor.getColorMapFilteredLastFrameFloat4(), // g_rayCast->getRayCastData().d_colors,
						g_sceneRep->getLastRigidTransform(),
						...
						GlobalCameraTrackingState::getInstance().s_colorGradientMin,
						...
						GlobalCameraTrackingState::getInstance().s_residualEarlyOut,
						g_RGBDAdapter.getDepthIntrinsics(),
...
// Registrating frame
void reconstruction_cameraposeopt()
{
	...
	if (GlobalRGBDReaderState::getInstance().s_bReadCameraPoseFromFile)
	{
		...
		if (g_RGBDAdapter.isFrameIdxInRangeOfRGBData(frameIdx))
		{
			...
			std::cout << "  Loading camera pose for frame " << frameIdx << std::endl << std::endl;
		}
		else
		{
			transformation = g_sceneRep->getLastRigidTransform();
		}		
	}
	if (g_RGBDAdapter.getFrameNumber() > 1)
	{
		...
		if (GlobalRGBDReaderState::getInstance().s_bRunPoseEstimation)
		{
			...
			else
			{
				...
				else
				{
					...
					case 1:
						// RGB-D tracking (seems does NOT work right now)
						transformation = g_cameraTrackingRGBD->applyCT(
							//g_rayCast->getRayCastData().d_depth4Transformed, g_CudaDepthSensor.getColorMapFilteredFloat4(),
							g_CudaDepthSensor.getCameraSpacePositionsFloat4(),
							g_CudaDepthSensor.getNormalMapFloat4(),
	...
	// CHAO NOTE: Originally only record the trajectory with corresponding flag true. Now always record trajectory matrix per frame.
	//if (GlobalAppState::getInstance().s_recordData) {
	//	g_RGBDAdapter.recordTrajectory(transformation);
	//}
...
void CALLBACK OnD3D11FrameRender( ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dImmediateContext, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	if (GlobalRGBDReaderState::getInstance().s_bRenderModel)
	{
