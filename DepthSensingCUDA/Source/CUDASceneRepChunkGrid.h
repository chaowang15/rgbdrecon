...
public:
	CUDASceneRepChunkGrid(CUDASceneRepHashSDF* sceneRepHashSDF, const vec3f& voxelExtends, const vec3i& gridDimensions, const vec3i& minGridPos, unsigned int initialChunkListSize, bool streamingEnabled, unsigned int streamOutParts)	{
		...
		m_maxNumberOfSDFBlocksIntegrateFromGlobalHash = 20000; // CHAO NOTE: original 100000 (will prompt cannot assign GPU memory if this parameter is too large)
																// Try increasing "s_SDFVoxelSize" in "zParametersDefault.txt" file to make your reconstructed model 
																// a little sparser if your GPU memory is small
